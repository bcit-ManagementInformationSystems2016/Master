...
public class ProjectManager {
	...
	public Project[] getSome(int empID) {
		TypedQuery<Project> query = em.createQuery("SELECT c FROM Project c WHERE ProjectManager = " + empID + "",
				Project.class);
		java.util.List<Project> categories = query.getResultList();
		Project[] catarray = new Project[categories.size()];
		for (int i = 0; i < catarray.length; i++) {
	...
	public Project[] getAllProjectsForEmp(int empID) {
		TypedQuery<ProjectEmployees> query = em.createQuery(
				"SELECT c FROM ProjectEmployees c WHERE EmployeeID = " + empID + "", ProjectEmployees.class);
		List<ProjectEmployees> wps = query.getResultList();
		Project[] proArray = new Project[wps.size()];
		for (int i = 0; i < proArray.length; i++) {
			TypedQuery<Project> empQuery = em.createQuery(
					"SELECT c FROM Project c WHERE ProjectID = " + wps.get(i).getPro().getProjectID() + "",
					Project.class);
	...
	public List<SelectItem> getYourProjects (int empID) {
			...
			List<ProjectEmployees> p = query.getResultList();
			List<SelectItem> pList = new ArrayList<SelectItem>();
			for (int i = 0; i < p.size(); i++) {
				pList.add(new SelectItem(p.get(i).getPro().getProjectID()));
			}
			return pList;	
		}
}